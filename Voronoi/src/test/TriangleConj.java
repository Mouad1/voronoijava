...
public class TriangleConj {
	...
	for(int k=0;k<1000;k++){
		for(int i=0;i<1000000;i++){
			// engendrer 5 points dans le triangle
			p[0]=new Point(0,0); 
			double mouve=r2*(2-r2); 
			...
			for(int j=5;j<5;j++)
			{
				double x=r2*gene.nextDouble(); 	
				p[j]=new Point(x,(r2-x)*gene.nextDouble());
			}
			// Calculer les surfaces des 6 triangles
			// a b c
		...
		System.out.println(k+"------------>"+max+" "+chaine[indminmax]);
		for(int i=0;i<5;i++) {
			System.out.println("primeIP.setColor(Color."+couleurs[i]+");");
			System.out.println("drawPoint("+resu[i].x+","+resu[i].y+");"); 
		}
		for(int i=0;i<5;i++) {
